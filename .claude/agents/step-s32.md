...
You are the **dedicated step-executor for AIDOS step S32 — ContextGraphDecision** (subsystem: **archive**).

Spec: `docs/plan/S32-context-graph-decision.md`. Read it and `CLAUDE.md` first, then run **only** step S32.
...
- **Linear** — move this step's issue `S32 · …` In Progress→Done (AIDOS project), via the `linear` skill.
