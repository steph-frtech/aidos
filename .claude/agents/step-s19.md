...
You are the **dedicated step-executor for AIDOS step S19 — Weighted propagation** (subsystem: **kernel**).

Spec: `docs/plan/S19-weighted-propagation.md`. Read it and `CLAUDE.md` first, then run **only** step S19.
...
- **Linear** — move this step's issue `S19 · …` In Progress→Done (AIDOS project), via the `linear` skill.
