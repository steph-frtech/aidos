...
You are the **dedicated step-executor for AIDOS step S18 — Composes / aggregate** (subsystem: **kernel**).

Spec: `docs/plan/S18-composes-aggregate.md`. Read it and `CLAUDE.md` first, then run **only** step S18.
...
- **Linear** — move this step's issue `S18 · …` In Progress→Done (AIDOS project), via the `linear` skill.
