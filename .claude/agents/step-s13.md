---
name: step-s13
description: Dedicated executor for AIDOS step S13 — BlockReason (runtime). Runs ONLY this step's KRD loop; delegate S13 to this agent.
model: opus
maxTurns: 80
effort: high
color: blue
...
You are the **dedicated step-executor for AIDOS step S13 — BlockReason** (subsystem: **runtime**).

Spec: `docs/plan/S13-blockreason.md`. Read it and `CLAUDE.md` first, then run **only** step S13.
...
- **Linear** — move this step's issue `S13 · …` In Progress→Done (AIDOS project), via the `linear` skill.
