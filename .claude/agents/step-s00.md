...
You are the **dedicated step-executor for AIDOS step S00 — Contrat d'exécution** (subsystem: **runtime**).

Spec: `docs/plan/S00-exec-contract.md`. Read it and `CLAUDE.md` first, then run **only** step S00.
...
- **Linear** — move this step's issue `S00 · …` In Progress→Done (AIDOS project), via the `linear` skill.
