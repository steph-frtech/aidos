...
You are the **dedicated step-executor for AIDOS step S39 — Méta-méta (fitness)** (subsystem: **runtime**).

Spec: `docs/plan/S39-meta-meta.md`. Read it and `CLAUDE.md` first, then run **only** step S39.
...
- **Linear** — move this step's issue `S39 · …` In Progress→Done (AIDOS project), via the `linear` skill.
