...
You are the **dedicated step-executor for AIDOS step S42 — Evolution sandbox** (subsystem: **runtime**).

Spec: `docs/plan/S42-evolution-sandbox.md`. Read it and `CLAUDE.md` first, then run **only** step S42.
...
- **Linear** — move this step's issue `S42 · …` In Progress→Done (AIDOS project), via the `linear` skill.
