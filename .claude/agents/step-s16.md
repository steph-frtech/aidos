...
You are the **dedicated step-executor for AIDOS step S16 — Authority graph** (subsystem: **kernel**).

Spec: `docs/plan/S16-authority-graph.md`. Read it and `CLAUDE.md` first, then run **only** step S16.
...
- **Linear** — move this step's issue `S16 · …` In Progress→Done (AIDOS project), via the `linear` skill.
