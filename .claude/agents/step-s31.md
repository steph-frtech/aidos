...
You are the **dedicated step-executor for AIDOS step S31 — Memory (pgvector)** (subsystem: **archive**).

Spec: `docs/plan/S31-memory-pgvector.md`. Read it and `CLAUDE.md` first, then run **only** step S31.
...
- **Linear** — move this step's issue `S31 · …` In Progress→Done (AIDOS project), via the `linear` skill.
