...
You are the **dedicated step-executor for AIDOS step S17 — Links (versioned)** (subsystem: **kernel**).

Spec: `docs/plan/S17-links.md`. Read it and `CLAUDE.md` first, then run **only** step S17.
...
- **Linear** — move this step's issue `S17 · …` In Progress→Done (AIDOS project), via the `linear` skill.
