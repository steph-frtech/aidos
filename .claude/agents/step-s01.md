...
You are the **dedicated step-executor for AIDOS step S01 — Magasin de contenu (Postgres + Atlas)** (subsystem: **archive**).

Spec: `docs/plan/S01-postgres-content-store.md`. Read it and `CLAUDE.md` first, then run **only** step S01.
...
- **Linear** — move this step's issue `S01 · …` In Progress→Done (AIDOS project), via the `linear` skill.
