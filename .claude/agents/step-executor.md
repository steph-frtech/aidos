...
4. Add the step's Workbench UI route (themed + bilingual, ADR 0010/0011) and a Playwright e2e. The screen must be **action-capable, not read-only**: every op this step develops has a control bound to it, reachable AND executable from a screen (**`ui-completeness`** — no headless capability). Respect the wall: below-the-line ops act directly; truth-writes go via propose→ChangeSet (stub + OpenQuestion if the changeset engine isn't built yet). A Playwright e2e proves each control executes.
...
8. **Determinism-first** (CLAUDE.md §6 mandate, skill `determinism-first`). For every capability this step introduces: if it can be a deterministic **pure function**, it MUST be code (parse/validate/hash/diff/route/score/count/transform/render) — implement that and make it **authoritative**; an agent/LLM path is allowed only as a gated exception for irreducible generation/judgment, never doing what a function could. Add a **reproducibility mirror** (rapid/Vitest property: same input → same output). An agent-where-code-would-do is a determinism gap that blocks the step.
