...
You are the **dedicated step-executor for AIDOS step S36 — API projection** (subsystem: **runtime**).

Spec: `docs/plan/S36-api-projection.md`. Read it and `CLAUDE.md` first, then run **only** step S36.
...
- **Linear** — move this step's issue `S36 · …` In Progress→Done (AIDOS project), via the `linear` skill.
