...
**Forward-dep OpenQuestions (non-blocking, by-design):** OQ-S13-scope (OUT_OF_SCOPE names role/owner generically; TruthScope record at S14); OQ-S13-fetch (live-block-fetch MCP later); OQ-S13-fold-s04 (S04's wall still declares its own BlockReason struct locally — folding it to import this package is a change to S04's interface needing a ChangeSet+SemanticDiff, documented at docs/plan/S13-changeset-s04-foldin.md, NOT a silent rewrite per §9). Linear unauth this run (OQ).

**Re-verified 2026-06-07 (independent):** enum now 29 codes (later steps folded into the one registry, each change_type:refine never removal); go test blockreason+cmd/aidos GREEN, Godog 7/31, property mirror prison-invariant lines 44-45, front vitest 8/8, tsc rc=0, biome 5 files clean, i18n 3441==3441 whyBlocked 16==16, docs.json:93-94 mint-validate PASS, clone 0 ahead origin/main d5c9a0b. WALL grep front = only explanatory text (no SQL/truth-write). verified-green ZERO corrections (this run and prior).
