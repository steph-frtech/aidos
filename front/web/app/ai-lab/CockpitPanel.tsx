...
export function CockpitPanel() {
	...
	return (
		<div className="mt-10 space-y-6">
			<div className="grid grid-cols-1 gap-6 lg:grid-cols-[36rem_1fr]">
								...
								>
									...
									<ol className="space-y-1.5">
										{MIRROR_PAIRS.map((pair) => {
											...
											const badge = placed
												? statusBadge(placed.status, t)
												: null;
											const canValidate =
												placed && placed.status !== "realized";
							...
							<div className="space-y-2 border-t border-border pt-3">
								...
								<ul className="space-y-1.5">
									{EXISTING_DAG.map((s) => {
										...
										const resolved =
											impacted && impactResolved(placements, s.level, s.facet);
										return (
											<li
												key={s.id}
			...
			{/* ── EN BAS — tout voir : le grand graphe (graphify) pleine largeur ── */}
			<section
			...
			>
				...
				<SpecGraph3D graph={graph} height={680} />
			</section>
		</div>
	);
}
