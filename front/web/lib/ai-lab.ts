/**
 * ai-lab — FK11 (ROADMAP-fke, FKE-38): the deterministic core of the AI Lab COCKPIT, the
 * trialogue screen `/ai-lab`. It is the pure twin of the cockpit's three panes and the single
 * load-bearing rule of the whole step: every gap, every scope, every transition shown is a
 * CALCULATION, never an LLM judgment (CLAUDE.md §6/§8). The cockpit COMPOSES the existing
 * truths — the FK09 conscience report (lib/conscience), the FK08 facet skeleton (lib/facetwire),
 * S58/S60 graph nav — it AUTHORS none.
 *
 * THE TRIALOGUE (FKE-38). Three panes, one screen at two zooms:
 *   - GAUCHE — the chat / vibe scoped to the open node (the « cerveau gauche »). It PROPOSES
 *     slots (amber, status="proposed"); it NEVER writes a truth. A chat message that asks to
 *     write the kernel/a mirror is REFUSED at the wall (`refuseTruthWrite`) — the only door is
 *     idea → mirror → /goal. A proposed slot is a candidate, content-addressed + idempotent.
 *   - CENTRE — the navigable layer: the kernel + its 1-for-1 anatomy, the WALL drawn between
 *     above-the-line (kernel/mirrors, propose-only) and below-the-line (projections, act
 *     directly), and a 🟢/🔴/🟡 voyant per PAIR of every instantiated facet (the live
 *     conscience). Below the wall is READ-ONLY from the cockpit (acts directly elsewhere; the
 *     cockpit only displays it).
 *   - DROITE — the decision cards (FK09) + the blast radius (SemanticDiff) + the red wave (S22)
 *     + the promotion gate (FK10 autonomy).
 *
 * TWO MODES (FKE-38). conversational vs navigational = the SAME screen at a different zoom — the
 * mode is a render hint, never a different truth; `cockpitState` carries both.
 *
