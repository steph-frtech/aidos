...
| **FN03 (émetteurs fonctionnels)** | « (Go/DDL/TS) » → **drop Go** pour le serveur émis, devient **(DDL/TS)** — TS pour handlers+workers+client, DDL pour Postgres. Miroir de pureté (même entrée→sortie, pas de var mutable de module, ré-émission byte-identique) s'applique à TS inchangé. **Réalise ADR 0040 Déc. 7 : le handler Hono appelle le service-interpréteur Go (client typé, outil MCP) ; AUCUN port TS de l'Operation-DSL.** |
