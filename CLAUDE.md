...
- **All execution agents run on Fable 5 (ultracode):** the orchestrator (`plan:parse`, pinned `model: 'fable'`), `step-executor`, `step-verifier`, `test-runner`, `suggestion-compat-checker`, `playwright-tester` — `model: fable`, effort xhigh. The project default is pinned in `.claude/settings.json` (`"model": "claude-fable-5"`), and every headless launch (cron/tmux scripts) passes `--model claude-fable-5 --effort xhigh` explicitly (the CLI rejects `ultracode` as an `--effort` value: ultracode = xhigh + workflow orchestration, carried by the prompt). The executor/verifier must **always emit their StructuredOutput report, even if incomplete** (a Sonnet executor twice burned its turn budget on S01 and never reported). Custom step agents carry **no strict `tools:` allowlist** (a strict list excludes the workflow's `StructuredOutput` tool and the web tools the per-step tool-search needs).
