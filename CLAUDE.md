...
- **The execution model is PARAMETERIZABLE — one source of truth (default Opus 4.8; Fable 5 was disabled 2026-06-13).** The project default lives in `.claude/settings.json` (`"model": "claude-opus-4-8"`, `"effortLevel": "xhigh"`); **all execution agents are `model: inherit`** (`step-executor`, `step-verifier`, `test-runner`, `suggestion-compat-checker`, `playwright-tester`, the 84 step/role agents) — they FOLLOW the session default, so switching the whole fleet is a one-line edit to `settings.json`. The orchestrator `plan:parse` is pinned `model: 'opus'`. Headless launches read **`AIDOS_BUILD_MODEL` (default `claude-opus-4-8`)** — the cron/tmux scripts pass `--model "${AIDOS_BUILD_MODEL:-claude-opus-4-8}" --effort xhigh`; runtime LLM calls (ai-lab/v2/v3 server actions) read **`AIDOS_LLM_MODEL` (default `claude-opus-4-8`)**. Every headless launch also carries the lowercase keyword `ultracode` in the prompt — the harness detects it and arms multi-agent workflow orchestration (the CLI rejects `ultracode` as an `--effort` value: headless ultracode = keyword + xhigh + workflow orchestration). **To switch models everywhere:** edit `settings.json` `model` (sessions + inherit agents) + set `AIDOS_BUILD_MODEL`/`AIDOS_LLM_MODEL` (headless + runtime) — no per-file sweep. The executor/verifier must **always emit their StructuredOutput report, even if incomplete** (a Sonnet executor twice burned its turn budget on S01 and never reported). Custom step agents carry **no strict `tools:` allowlist** (a strict list excludes the workflow's `StructuredOutput` tool and the web tools the per-step tool-search needs).
