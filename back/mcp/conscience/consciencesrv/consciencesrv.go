// Package consciencesrv is the AIDOS Runtime conscience MCP server (FK09; ADR 0009: every backend
...
// that composes the verdicts of the EXISTING judges (the mirror runner, the completeness/monster
// law, SemanticDiff, the RealityMirror, the sensors, the ledger) into ONE ConsciousnessReport per
// kernel + the §FKE-31 decision cards. AUCUN NOUVEAU JUGE (FKE-6.3): the conscience adds no
// judgment — it READS sourced verdicts and ROUTES them. Two pure, write-nothing tools:
//
...
// second agent that validates). Every tool's I/O is a JSON OBJECT (no json.RawMessage), so the S59
// gateway dispatches both synchronously over the in-memory transport (the HTTP honours the MCP).
//
...
// standalone stdio binary (back/mcp/conscience) and the dispatcher construct identical behaviour —
// no duplicated logic, no twin (reuse, don't reinvent — CLAUDE.md §0).
package consciencesrv
...
// NewServer builds the configured conscience *mcp.Server and registers the two pure aggregator
// tools — identical behaviour whether driven by the standalone stdio binary or the S59 gateway
// dispatcher over an in-memory transport. It takes no deps: each tool composes existing verdicts
// over its input (AUCUN NOUVEAU JUGE).
