// Package whytreesrv is the AIDOS Runtime WhyTree MCP server (FK13; ADR 0009: every backend op
...
// judgment; the judge is the reproduction bool). Every tool's I/O is a JSON OBJECT (no
// json.RawMessage), so the S59 gateway dispatches all three synchronously over the in-memory
// transport (the HTTP honours the MCP).
//
...
// standalone stdio binary (back/mcp/why-tree) and the dispatcher construct identical behaviour —
...
func toInput(in buildIn) whytree.Input {
...
func build(_ context.Context, _ *mcp.CallToolRequest, in buildIn) (*mcp.CallToolResult, buildOut, error) {
	tree, err := whytree.Build(toInput(in))
	if err != nil {
...
// NewServer builds the configured WhyTree *mcp.Server and registers the three pure tools —
// identical behaviour whether driven by the standalone stdio binary or the S59 gateway dispatcher
// over an in-memory transport. It takes no deps: every tool is a pure graph-walk over its input.
