...
// backend op is an MCP tool). It is the capability door over the S80 `app-auth` behavior-macro
...
// actual kernel freeze is the `aidos` CLI's job downstream. Transport: stdio.
package main
...
// newMCPServer registers the three S80 app-auth tools. Every tool is PURE; `attach` lands via
