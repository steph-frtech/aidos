// Package buildconsolesrv is the reusable AIDOS Workbench build-console MCP server library
// (S86; ADR 0009: every backend op is an MCP tool). Extracted from back/mcp/build-console
// (ADR 0092, batch-2) so the S58 gateway dispatcher reuses the SAME in-process server as the
// stdio binary — reuse, don't reinvent (CLAUDE.md §0); the engine is the SINGLE live source.
//
...
// NewServer builds the deterministic, dependency-free build-console MCP server. Gateway +
