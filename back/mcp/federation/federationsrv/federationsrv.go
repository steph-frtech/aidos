// Package federationsrv is the AIDOS Runtime cross-cell federation MCP server (S103; ADR 0009:
// every backend op is an MCP tool), exposed as a LIBRARY (S59 dispatcher reuse).
//
...
// runtime.red_work_queue is the S22 PostKernelChange hook's job below the waterline (NEVER
// from this tool — fan_out returns the computed waves, it does not enqueue them).
//
...
// mirror). Every tool's I/O is a JSON OBJECT (no json.RawMessage), so the S59 gateway
...
// the standalone stdio binary (back/mcp/federation) and the dispatcher construct identical
...
// NewServer builds the configured federation *mcp.Server and registers the three pure cross-cell
// tools — identical behaviour whether driven by the standalone stdio binary or the S59 gateway
// dispatcher over an in-memory transport. It takes no deps: every tool is a pure composition over
// its input.
