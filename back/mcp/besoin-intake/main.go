...
//
// It is the SINGLE capability door over the BesoinGraph (ADR 0009: every backend op is an MCP tool) —
// the NEED store ABOVE the wall (CLAUDE.md §2), DISTINCT from the truth-store (kernel/mirrors/fitness),
// NOT an exception to the wall. A human EXPLAINS their app level by level (the §23 verticale +
// the invariant/policy bands); each resolved level leaves as an Idea via the ONLY legal door,
// idea_capture (provenance human, the utterance verbatim, status draft). There is NO
// idea_promote_to_kernel here, NO kernel/mirrors/fitness write anywhere on this path.
//
...
//
//	READ / STATE:
//	  besoin_graph_state   — the current graph + the enterable level (EL07) + the per-level verdicts.
//	  besoin_level_schema  — the required fields + the outgoing ref of a level (a client renders the
//	                         right form, invents no field).
//	  besoin_list          — list the projects' captured node rows (the history depth), RLS-scoped.
//	CAPTURE per rung (each validates against besoin_level_schema, passes the EL07 gate, appends a
//	  LevelNode; a MAPPING rung (EL05) ALSO captures the Idea via idea_capture; a NoEmit rung
//	  (journey/view) appends + seeds anchors with NO Idea — no silent cast):
//	  besoin_capture_product/_journey/_view/_control/_action/_operation/_entity/_invariant
//	VALIDATE:
//	  besoin_validate_level — the pure pre-flight EL07 verdict (no write).
//	  besoin_classify       — the four-metadata classification (EL04, wraps classify-truth).
//	EMIT (EL16):
//	  besoin_emit_ideas     — project the whole graph into the backlog of draft Ideas (one per resolved
//	                          MAPPING rung, NoEmit excluded), governed by LevelToProposes (EL05); dry_run
//	                          previews. Persists via idea_capture (idempotent), never the kernel.
//
// THE WALL: fail-closed. A capture at a not-yet-enterable level is REFUSED with a BlockReason; the
// graph is unchanged on any refusal. Transport: stdio. DSN comes from AIDOS_BESOIN_DSN.
package main
...
// EL05 emission door, reusing ideas.idea). NO LLM enters: every routing/verdict is the pure besoin.*.
type server struct {
	store *Store
	ideas *ideaStore
}
...
// newMCPServer builds the MCP server and registers the besoin-intake tools (the 13 capture/read/
// validate tools + the EL16 besoin_emit_ideas batch emitter). There is NO promote-to-kernel tool: a
// need leaves only as an Idea (EL05/EL16), promotion is the /goal flow (S64).
...
func main() {
	...
	defer st.Close()
	is, err := newIdeaStore(ctx, dsn)
	if err != nil {
		log.Fatal(fmt.Errorf("besoin-intake: open idea store: %w", err))
	}
	defer is.Close()

	srv := newMCPServer(&server{store: st, ideas: is})
