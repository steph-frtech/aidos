// Package truthlevelsrv is the AIDOS Kernel truth-level MCP server (FK01; ADR 0009: every
// backend op is an MCP tool), exposed as a LIBRARY (S59 dispatcher reuse, ADR 0092).
//
// It is the capability door over FK01 (back/kernel/truthlevel): the seven KRD truth levels
// (FKE-5, Raw→Reconciled) and their deterministic transition:
//
...
// three synchronously over the in-memory transport (the HTTP honours the MCP — the S59
// byte-array transport scar is avoided by construction).
//
...
// the standalone stdio binary (back/mcp/truth-level) and the dispatcher construct identical
...
// NewServer builds the FK01 truth-level MCP server (the three tools above). Reused by BOTH
