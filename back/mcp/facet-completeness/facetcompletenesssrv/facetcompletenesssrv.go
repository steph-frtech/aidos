// Package facetcompletenesssrv is the AIDOS Mirror facet-aware completeness MCP server (FK04;
// ADR 0009: every backend op is an MCP tool), exposed as a LIBRARY (S59 dispatcher reuse, ADR 0092).
//
...
// no I/O, never an LLM. Same cut → same verdict (the FK04 done-criteria). Every tool's I/O is a
// JSON OBJECT (no json.RawMessage), so the S59 gateway dispatches it synchronously over the
// in-memory transport (the HTTP honours the MCP — the byte-array transport scar is avoided).
//
...
// the standalone stdio binary (back/mcp/facet-completeness) and the dispatcher construct
// identical behaviour — no duplicated logic, no twin (reuse, don't reinvent — CLAUDE.md §0).
...
// NewServer builds the FK04 facet-aware completeness MCP server (the `check` tool). Reused by
// BOTH the standalone stdio binary AND the S59 gateway dispatcher.
