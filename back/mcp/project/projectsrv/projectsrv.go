// Package projectsrv is the AIDOS Kernel/Archive `project` MCP server, exposed as a
// LIBRARY (S59 dispatcher reuse). It is the single capability door (ADR 0009: every
// backend op is an MCP tool) over the `projects` schema — the first-rank multi-tenant
// root scope under which every later truth lives (app-builder EPIC 1). A project is
// BELOW the wall (CLAUDE.md §2): the projects schema is not kernel/mirrors/fitness, it
// is content-addressed and append-only, so this server writes it directly via the store
// path (like the Archive content-store). It NEVER writes kernel/mirrors/fitness, and it
// offers NO hard-delete tool — soft delete only, append-only (the hard GDPR delete is
// S116).
//
...
// server only persists an already-decided, content-addressed row.
//
// WHY A LIBRARY (S59). The gateway dispatcher (back/runtime/gatewaydispatch) reuses this
// SAME server in-process: it builds the project *mcp.Server via NewServer and dispatches
// a routed below-the-line project_* call to it over an in-memory transport. Extracting
// the handlers + the store here (rather than the old package-main) lets BOTH the
// standalone stdio binary (back/mcp/project) and the dispatcher construct identical
...
// server wires the MCP tools to one project Store. It is unexported: callers construct
...
// NewServer builds the configured project *mcp.Server over a single project Store. It
// registers the six capability-door tools (create/list/get/archive/restore/delete) —
// identical behaviour whether driven by the standalone stdio binary or the S59 gateway
// dispatcher over an in-memory transport. There is deliberately NO hard-delete tool: a
// project is soft-deleted (kept, append-only); the hard GDPR delete is S116.
