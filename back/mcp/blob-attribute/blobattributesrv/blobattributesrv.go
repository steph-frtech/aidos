// Package blobattributesrv is the AIDOS Kernel BLOB / FILE attribute MCP server (S72; ADR 0009:
...
// blockreason; the handler is byte-stable. Every tool's I/O is a JSON OBJECT (no
// json.RawMessage), so the S59 gateway dispatches them synchronously over HTTP (the byte-array
// transport scar is avoided).
//
// WHY A LIBRARY (S59). The gateway dispatcher (back/runtime/gatewaydispatch) reuses this SAME
// server in-process; the standalone stdio binary (back/mcp/blob-attribute) and the dispatcher
// construct identical behaviour from one source — no duplicated logic, no twin (CLAUDE.md §0).
package blobattributesrv
...
// NewServer builds the blob-attribute MCP server. The handlers are PURE; the gateway dispatcher
