// Package deploysrv is the AIDOS S96 PHASE-KEYED DEPLOY MCP server (ADR 0009: every backend op is
// an MCP tool; app-builder EPIC 10, DP26 / ADR 0043), exposed as a LIBRARY (S59 dispatcher reuse).
//
...
// BlockReason. Every tool's I/O is a JSON OBJECT (no json.RawMessage), so the S59 gateway
// dispatches them synchronously over HTTP (the byte-array transport scar is avoided).
//
// WHY A LIBRARY (S59). The gateway dispatcher (back/runtime/gatewaydispatch) reuses this SAME
// server in-process; the standalone stdio binary (back/mcp/deploy) and the dispatcher construct
...
// NewServer builds the deploy MCP server. The handlers are PURE; the gateway dispatcher and the
