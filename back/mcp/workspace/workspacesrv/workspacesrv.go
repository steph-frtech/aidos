// Package workspacesrv is the AIDOS S82 PER-PROJECT SANDBOX MCP server, exposed as a LIBRARY
// (S59/ADR 0092 dispatcher reuse). It is the capability door over back/runtime/workspace — the
// isolated per-project workspace (container + git/jj repo + worktree + CPU/mem/disk/time limits)
// where a project's generated code lives, compiles and runs its mirrors. Four tools:
//
...
// holds no DB, no clock, no RNG, and never touches kernel/mirrors/fitness (the wall). A workspace
// writes NO truth: its root is below the line; the truth-store is OUTSIDE every workspace root
// (reaching it is exactly a SANDBOX_ESCAPE; WroteKernel is always false). Every I/O is a scalar
// OBJECT (no json.RawMessage body — the S59 byte-array transport scar is avoided by construction).
//
// WHY A LIBRARY (S59/ADR 0092). The gateway dispatcher reuses this SAME server in-process via NewServer
// and dispatches the routed below-the-line workspace_* read calls to it. Extracting the handlers here
// lets BOTH the standalone stdio binary (back/mcp/workspace) and the dispatcher construct identical
...
// NewServer registers the four S82 workspace tools. Every tool is PURE; provisioning is a dry-run
// value (WroteKernel always false); the truth-store is outside every workspace (the wall). Reused
// identically by the standalone stdio binary AND the gateway dispatcher (S59) — no twin.
