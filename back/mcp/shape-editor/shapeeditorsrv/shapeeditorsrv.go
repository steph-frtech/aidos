// Package shapeeditorsrv is the AIDOS Mirror shape-editor MCP server, exposed as a LIBRARY (S59/ADR
// 0092 batch-4B dispatcher reuse). It is the capability door over S68 (back/runtime/shapeeditor): a
// human authors a mirror by its FORM, the form is DERIVED from the truth-nature, the source is PARSED
// by a pure parser, and the red, project-scoped mirror is wrapped as a DRAFT ChangeSet PROPOSAL. Two
// concurrent edits of the same draft MERGE (disjoint) or LOCK (same-field clash) — never last-write-wins.
//
...
//
// THE WALL (CLAUDE.md §2): this server is PURE COMPUTATION — it returns the derivation, the parse, the
// merge verdict, and the DRAFT ChangeSet proposal as VALUES, and writes NOTHING. Persistence of the
// proposed DRAFT rides the changeset door (S20) under human approval; freezing into the mirrors schema
// stays the /goal flow. There is deliberately no apply tool.
//
// THE DISPATCH SURFACE (S59/ADR 0092). The gateway dispatcher dispatches only the THREE pure READ tools
// — shape_derive · shape_parse · shape_merge — each a CHEAP/pure compute whose I/O is a scalar OBJECT
// (no json.RawMessage body — the S59 byte-array transport scar avoided by construction). shape_propose
...
// /shape-editor panel keeps its propose→ChangeSet voie propre (the wall).
//
// DETERMINISM-FIRST (CLAUDE.md §6/§8): every tool is a PURE function of its input — no clock, no rng, no
// I/O. Shape selection + parsing are pure functions, never an LLM.
//
// WHY A LIBRARY (S59/ADR 0092). Extracting the handlers here lets BOTH the standalone stdio binary
// (back/mcp/shape-editor) and the gateway dispatcher construct identical behaviour — no twin (CLAUDE.md §0).
package shapeeditorsrv
...
// ── shape_propose (EXPOSED, not dispatched — the RawMessage scar; propose → ChangeSet → approval) ──
...
// NewServer builds the shape-editor MCP server and registers the four S68 tools. Every tool is PURE;
// shape_propose is born red and writes nothing (the wall). Reused identically by the standalone stdio
// binary AND the gateway dispatcher (S59) — no twin.
