// Package previewsrv is the AIDOS S94 EPHEMERAL PREVIEW ENVIRONMENT MCP server (ADR 0009: every
// backend op is an MCP tool; app-builder EPIC 10, ADR 0043 / DP25), exposed as a LIBRARY (S59
// dispatcher reuse, ADR 0092).
//
// It is the capability door over runtime/preview: the DETERMINISTIC planner that turns the EMITTED
// SURFACE of a content-addressed STABLE PHASE (the emitted server/front/infra/datastore hashes + the
// emitted Pulumi program) into a content-addressed PreviewPlan — the preview URL keyed on the phase, the
// `pulumi up` boot, the deterministic `pulumi destroy` teardown — and judges the done-criterion (the
// served-app hash equals the emitted-app hash of the phase). Tools:
//
//	plan         — Input → content-addressed PreviewPlan. PURE, same Input → byte-identical Plan.
//	app_hash     — the EmittedAppHash of a phase's surface (server ⊕ front ⊕ infra ⊕ datastore).
//	check_served — assert a running preview's served-app hash EQUALS the plan's emitted-app hash.
//
// THE WALL (CLAUDE.md §2): pure planning + a pure hash comparison over supplied bytes — writes NOTHING
// to the kernel/mirrors/fitness. A malformed input is a typed BlockReason. Every tool's I/O is a scalar
// OBJECT (preview.Input/PreviewPlan/EmittedSurface/PhaseRef are scalar — no json.RawMessage, no []byte),
// so the S59 gateway dispatches all three synchronously over HTTP (the byte-array scar avoided).
//
// WHY A LIBRARY (S59). The gateway dispatcher reuses this SAME server in-process; extracting the handler
// here (rather than the old package-main) lets BOTH the standalone stdio binary (back/mcp/preview) and
// the dispatcher construct identical behaviour — no duplicated logic, no twin (CLAUDE.md §0).
package previewsrv
...
// NewServer builds the configured preview *mcp.Server and registers the three pure tools — identical
