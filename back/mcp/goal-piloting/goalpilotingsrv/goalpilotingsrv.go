// Package goalpilotingsrv is the AIDOS Runtime goal-piloting MCP server (S66; ADR 0009: every
// backend op is an MCP tool), exposed as a LIBRARY (S59 dispatcher reuse).
//
// It is the capability door over the S66 UI-piloted /goal (back/runtime/goalpiloting): a human
// PORTEUR D'AUTORITÉ (S63) opens a goal from a grilled idea — the engine proposes a DRAFT
// ChangeSet (spec_delta + mirror_delta atomically) and computes the LIVE red set; and the close
// gate is the NON-GAMEABLE stop (red set→green ∧ prior intact ∧ mutation ≥ floor ∧ no monster).
// The actor gate refuses a placeholder/agent — a goal is never opened by nobody (the wall).
//
// THE WALL (CLAUDE.md §2): this server is PURE COMPUTATION — it returns the DRAFT ChangeSet
// PROPOSAL (by REFERENCE: id/status, NOT the raw envelope body) + the red set + the close verdict
// as VALUES and writes NOTHING. Persistence of the proposed ChangeSet rides the changeset door
// (S20) under human approval; stamping a goal CLOSED stays the aidos CLI role. There is
// deliberately no apply/close tool: applying is S20's commit-gate, closing is the aidos role.
//
// Tools (one tool = one backend op):
//
//	goal_pilot_open   — actor + open input → a DRAFT ChangeSet proposal (ref) + the LIVE red set (or a refusal)
...
// rng, no I/O. "Done" is computed, never declared; no agent-confidence is ever an input. Same
// input → same output. Every tool's I/O is a JSON OBJECT (the openOutput surfaces the ChangeSet by
// id/status strings, never the json.RawMessage-bearing envelope body), so the S59 gateway
// dispatches all three synchronously over the in-memory transport (the HTTP honours the MCP).
//
...
// standalone stdio binary (back/mcp/goal-piloting) and the dispatcher construct identical
...
// open is the goal_pilot_open tool: the actor gate (S63) + the open (S29). It returns the DRAFT
// ChangeSet PROPOSAL (by id/status reference) + the LIVE red set as VALUES, or an actionable
// refusal. Writes NOTHING; persistence rides the changeset door under approval (the wall). PURE.
...
// close is the goal_pilot_close tool: the NON-GAMEABLE close gate. It returns whether the goal is
// closeable (and the GOAL_STILL_RED refusal when not). It never stamps CLOSED (that stays the
// aidos role) and takes NO agent-confidence input — "done" is computed. PURE.
func closeGoal(_ context.Context, _ *mcp.CallToolRequest, in closeInput) (*mcp.CallToolResult, closeOutput, error) {
...
// redSet is the goal_live_red_set tool: the LIVE red-set worklist in stable sorted order — the
// panel's worklist. PURE.
...
// NewServer builds the MCP server and registers the three goal-piloting tools. There is
// deliberately NO apply/close-stamp tool: applying is S20's commit-gate, closing is the aidos role
// — this server is pure computation (the wall). Identical behaviour whether driven by the
// standalone stdio binary or the S59 gateway dispatcher over an in-memory transport.
func NewServer() *mcp.Server {
	...
	mcp.AddTool(srv, &mcp.Tool{Name: "goal_pilot_close", Description: "S66: the NON-GAMEABLE close gate — is the goal closeable? red set→green ∧ prior green intact ∧ mutation ≥ floor ∧ no monster. Returns the GOAL_STILL_RED refusal when not. Never stamps CLOSED; no agent-confidence input."}, closeGoal)
