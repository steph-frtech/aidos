// Package behaviorexpandersrv is the AIDOS Kernel behavior-expander MCP server (S76; ADR 0009:
// every backend op is an MCP tool), exposed as a LIBRARY (S59 dispatcher reuse, ADR 0092).
//
// It is the capability door over S76 (back/kernel/behavior): the behavior RECORD (ownable, versioned,
// taggable, localizable) and the ONE authoritative, deterministic Expand — the single function that
// turns a behavior attached to an entity into its attributes / relations / operations / policies /
// fixtures, and Propose, which wraps that dry-run into a DRAFT ChangeSet (never applied).
//
...
// expanded source into the kernel stays the /goal flow.
//
// S59 DISPATCH NOTE (ADR 0092). The three CHEAP READS (behavior_catalogue/validate_record/expand) carry
// only scalar OBJECT I/O (no json.RawMessage / []byte — the behavior.* pieces are scalar) → they
// dispatch synchronously over HTTP. behavior_propose stays EXPOSED by this server (the stdio binary +
// CI use it) but is a truth-PROPOSAL the front never fires synchronously: it is the propose → ChangeSet
// → approval door, so the panel keeps that voie propre (route(behavior_propose) → unknown_tool, the
...
// behavior-expander) and the dispatcher construct identical behaviour — no duplicated logic, no twin
...
// NewServer builds the MCP server and registers the four S76 tools — identical behaviour whether driven
// by the standalone stdio binary or the S59 gateway dispatcher. NO apply tool: applying the DRAFT is
// S20's commit-gate, freezing is the /goal flow — this server is pure computation (the wall).
