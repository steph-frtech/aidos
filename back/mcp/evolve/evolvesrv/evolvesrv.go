// Package evolvesrv is the AIDOS Runtime EvolutionSandbox MCP server, exposed as a
// LIBRARY (S59 dispatcher reuse).
//
...
//	evolve_coverage          — measure the EG04 niche-coverage win of each generator.
//
...
//
// WHY A LIBRARY (S59). The gateway dispatcher (back/runtime/gatewaydispatch) reuses
// this SAME server in-process: it builds the evolve *mcp.Server via NewServer and
// dispatches a routed below-the-line evolve_* call to it over an in-memory transport.
// Extracting the handlers here (rather than the old package-main) lets BOTH the
// standalone stdio binary (back/mcp/evolve) and the dispatcher construct identical
// behaviour — no duplicated logic, no twin (reuse, don't reinvent — CLAUDE.md §0).
//
// DETERMINISM-FIRST (CLAUDE.md §6/§8): the dispatch path uses NewServer (the pure
// deterministic sampler + in-memory store) — NEVER the gated LLM self-play (that is
// armed only by the standalone binary's env, never in the dispatch loop).
package evolvesrv
...
// newServer builds the DETERMINISTIC EvolutionSandbox server: an in-memory run store + the
// pure deterministic sampler. NO DSN, NO LLM — this is the form the gateway dispatcher reuses
// in-process (the dispatch path never arms self-play; determinism-first, CLAUDE.md §6/§8).
...
// remains the fallback authority (used when the Proposer yields nothing). This is the GATED
// LLM exception (§6) — used only by the standalone stdio binary, never the dispatch path.
...
// NewServer builds the standalone EvolutionSandbox *mcp.Server (the DETERMINISTIC form: the
// in-memory run store + the pure deterministic sampler). It takes NO DSN — the EvolutionSandbox
// persists runs only to its injected in-memory store (the seam where the aidos CLI dag/ideas
// write-grant would land them) and judges with the pure evolve engine. This is the constructor
// BOTH the stdio binary (its default path) and the S59 gateway dispatcher use in-process: the
// wall + the determinism are identical (no twin — CLAUDE.md §0).
func NewServer() *mcp.Server {
	return newMCPServer(newServer())
}

// NewServerWithSelfPlay builds the *mcp.Server with the EG03 self-play generator armed behind
// the frozen seam (the gated LLM exception, §6), injecting the given Proposer (production:
// evolve.ClaudeProposer = real CLI + deterministic fallback; tests: evolve.FixtureProposer =
// hermetic). The deterministic sampler remains the fallback. The standalone binary uses this
// when AIDOS_EVOLVE_SELFPLAY=1; the gateway dispatcher NEVER does (no LLM in the dispatch path).
func NewServerWithSelfPlay(proposer evolve.Proposer) *mcp.Server {
	return newMCPServer(newServerWithSelfPlay(proposer))
}
