// Package behaviorcapturesrv is the AIDOS Runtime behavior-capture MCP server (S67; ADR 0009:
// every backend op is an MCP tool), exposed as a LIBRARY (S59 dispatcher reuse, ADR 0092).
//
// It is the capability door over S67 (back/runtime/behaviorcapture): at idea-capture the reusable
// behaviours library (S79/S76) is surfaced; attaching one DRY-RUN-EXPANDS it — in attributes /
// relations / operations / policies / fixtures — as a DRAFT ChangeSet PROPOSAL, produced by the ONE
// authoritative behavior.Expand (S76), never a second implementation.
//
...
//
// THE WALL (CLAUDE.md §2): this server is PURE COMPUTATION — it returns the surfaced library, the
// dry-run expansion + its DRAFT ChangeSet proposal as VALUES, and writes NOTHING. Persistence of the
// proposed DRAFT rides the changeset door (S20) under human approval; freezing the expanded source into
// the kernel stays the /goal flow. There is deliberately no apply tool. Every dispatched tool's I/O is
// a scalar OBJECT (attachOutput carries the ChangeSet by string ref/status + the behavior.* pieces are
// scalar — no json.RawMessage, no []byte), so the S59 gateway dispatches both synchronously over HTTP.
//
...
// output (the package property mirror pins it).
//
// WHY A LIBRARY (S59). The gateway dispatcher reuses this SAME server in-process; extracting the handler
// here (rather than the old package-main) lets BOTH the standalone stdio binary (back/mcp/
// behavior-capture) and the dispatcher construct identical behaviour — no duplicated logic, no twin
// (reuse, don't reinvent — CLAUDE.md §0).
...
// NewServer builds the MCP server and registers the two behavior-capture tools — identical behaviour
// whether driven by the standalone stdio binary or the S59 gateway dispatcher. There is deliberately
// NO apply tool: applying the DRAFT is S20's commit-gate, freezing is the /goal flow — this server is
// pure computation (the wall).
