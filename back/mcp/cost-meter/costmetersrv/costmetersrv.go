// Package costmetersrv is the reusable AIDOS Runtime COST-METER MCP server library (S111;
// ADR 0009: every backend op is an MCP tool). Extracted from back/mcp/cost-meter (ADR 0092,
// batch-2) so the S58 gateway dispatcher reuses the SAME in-process server as the stdio binary —
// reuse, don't reinvent (CLAUDE.md §0); the engine is the SINGLE live source.
//
...
// lib/cost-meter.test.ts) pin it. No DSN, no clock, no embedder — dispatch-safe (every I/O is
// a scalar object; no json.RawMessage body, the S59 byte-array transport scar avoided).
...
// NewServer builds the deterministic, dependency-free COST-METER MCP server. The gateway
// dispatcher and the stdio binary both call it (one server, no twin). Pure: no DSN, no clock.
