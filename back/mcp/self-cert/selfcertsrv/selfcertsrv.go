// Package selfcertsrv is the AIDOS Runtime build-loop SELF-CERTIFICATION MCP server, exposed as a
// LIBRARY (S59/ADR 0092 dispatcher reuse). It is the capability door over the S84 self-certification
// battery (back/runtime/buildloop/selfcert): the deterministic SENSOR BATTERY that gates EVERY diff
// the build-loop service (S83) takes. Given a set of per-sensor verdicts over a candidate diff
// (types/lint/unit/fixtures/properties/Pact contract/arch-fitness of the emitted TS tree via
// dependency-cruiser), the server CERTIFIES the diff: the iteration is GREEN only when every sensor
// passes; a single red sensor BLOCKS the iteration before green, with the actionable
// BUILD_LOOP_SENSOR_RED BlockReason.
//
// THE WALL (CLAUDE.md §2): this server is PURE COMPUTATION — it folds the sensor verdicts into the
// battery report as VALUES and writes NOTHING. The sensor RUNS (tsc/Vitest/go test/dependency-cruiser…)
// happen in the build adapter over the sandbox (below the line, S82); this door judges their results.
// The kernel/mirrors/fitness are SELECT-only to the agent; there is no truth-write tool (WroteKernel is
// always false) — a green build proposes its truth through propose→ChangeSet (S85).
//
// DETERMINISM-FIRST (CLAUDE.md §6/§8): every tool is a PURE function of its input — the judge is the
// deterministic mirror, never the LLM. Same verdicts → same battery (anti-passthrough — a missing
// sensor is RED, never silently green). Every I/O is a scalar OBJECT (no json.RawMessage body — the
// S59 byte-array transport scar is avoided by construction).
//
// WHY A LIBRARY (S59/ADR 0092). The gateway dispatcher (back/runtime/gatewaydispatch) reuses this SAME
// server in-process via NewServer and dispatches the routed below-the-line selfcert_* read calls to it
// over an in-memory transport. Extracting the handlers here lets BOTH the standalone stdio binary
// (back/mcp/self-cert) and the dispatcher construct identical behaviour — no twin (reuse, don't
...
// NewServer builds the self-cert MCP server and registers the three S84 self-certification tools.
// Every tool is PURE; the judge is the mirror, never the LLM (the wall). Reused identically by the
