// Package backtestersrv is the AIDOS Runtime out-of-sample evaluation MCP server, exposed as
// a LIBRARY (S59 dispatcher reuse). (S42, KRD §62 ② out_of_sample, §87.)
//
...
// (Promote) consumes — it is a READ/EVALUATE capability: it returns a fitness READING, it does
// NOT define or edit the fitness (the metre-stick is held above the line, never by the loop —
// CLAUDE.md §8).
//
...
//
// THE WALL (CLAUDE.md §2/§8): this server reads/evaluates only — it writes NOTHING above the
// line and holds NO grant on kernel/mirrors/authority/fitness. It never defines the fitness;
// it reports a reading the gate consumes.
//
// INJECTION SEAM: a deterministic evaluator backs the tool so the Workbench can exercise the
// gate without a real market/data feed. The real out-of-sample backtest (real data,
// walk-forward) lives behind this seam; the verdict shape is the contract.
//
// DISPATCH NOTE (S59). backtest_get is a CHEAP READ — it reads a recorded verdict from the
// in-memory store (no external feed, no subprocess), so the gateway dispatches it
// synchronously. backtest_out_of_sample stays exposed by the server (the standalone binary +
// the gate use it) but is the EVALUATE op behind the data-feed seam; the front does not
// dispatch it synchronously (the determinism-first read is backtest_get).
//
// WHY A LIBRARY (S59). The gateway dispatcher (back/runtime/gatewaydispatch) reuses this SAME
// server in-process via NewServer over an in-memory transport. Extracting the handlers here
// (rather than the old package-main) lets BOTH the standalone stdio binary
// (back/mcp/backtester) and the dispatcher construct identical behaviour — no twin (reuse,
...
// OutOfSampleVerdict is the honest signal (KRD §87): green ⇒ the variant held up out of
// sample; red ⇒ it did not. Mirrors evolve.OutOfSampleStatus's vocabulary.
...
type backtestInput struct {
	...
	// InSample is REJECTED: the backtester evaluates out-of-sample ONLY (§87). The field
	// exists so a caller cannot smuggle in-sample data unnoticed; a true value is refused with
	// a reason.
	InSample bool `json:"in_sample" jsonschema:"must be false — the backtester evaluates OUT-OF-SAMPLE only (§87)"`
	// Score is the injected out-of-sample reading [0,1] the deterministic evaluator uses (the
	// real evaluator would compute it from walk-forward over held-out data).
...
// the declared out-of-sample bar (above the line; the agent is graded by it, never authors
// it). Declared, not learned (CLAUDE.md §8).
...
// server wires the MCP tool handlers to one in-memory evaluation store. It is unexported:
// callers construct the configured *mcp.Server via NewServer.
type server struct {
	mu    sync.Mutex
...
// NewServer builds the configured backtester *mcp.Server over a fresh in-memory evaluation
// store. It registers the two capability-door tools (backtest_out_of_sample/backtest_get) —
// identical behaviour whether driven by the standalone stdio binary or the S59 gateway
// dispatcher over an in-memory transport. It takes no deps (no DSN, no clock).
