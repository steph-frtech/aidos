// Command ops-observability is the AIDOS S92 OPS-OBSERVABILITY MCP server (app-builder
// EPIC 9 / E12, ROADMAP-app-builder §S92, DP17, ADR 0009: every backend op is an MCP
// tool). It is the capability door over the S92 per-app ops engine
// (back/runtime/opsobservability): the emitted app's OpenTelemetry signals (logs,
// spans, error events) flow in; a per-app ops DASHBOARD (latency, error rate, logs,
...
// never touches kernel/mirrors/fitness (the wall). Transport: stdio.
package main
...
// newMCPServer registers the three S92 ops-observability tools. Every tool is PURE and
// writes nothing (the wall) — ops-observability is a render layer, never a Kernel
// on-ramp (the E12 RealityMirror is the only on-ramp).
