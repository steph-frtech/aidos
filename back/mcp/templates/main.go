// Command templates is the AIDOS S81 CURATED TEMPLATE CATALOGUE MCP server (ADR 0009: every backend op
// is an MCP tool). It is the capability door over back/kernel/templates — the curated starter apps
// (e-commerce, CRM, booking) packaged as content-addressed bundles. Four tools:
//
...
// (propose → ChangeSet → approval), which is templates.Propose (exposed by the gateway, not mutated
// here). The starter-project ROW (a duplicate-from-template) is written below the line by the project/
// DAG store path (S56), not by this pure server. Transport: stdio.
package main
...
// newMCPServer registers the four S81 templates tools. Every tool is PURE; instantiate/fork are dry-run
// value computations, never a direct kernel write (the wall).
