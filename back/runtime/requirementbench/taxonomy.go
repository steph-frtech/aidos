// taxonomy.go — DG02: the CLOSED requirement-type taxonomy + the PURE extractor that maps an LLM
// output to the SET of requirement TYPES it surfaces. This is the determinism-first heart of the
// RequirementBench port (ADR 0088): the CompletenessReport is a COUNT of TYPES, never a quality
// judgement of generated code (§8 forbids an LLM judgement hoisted to truth). The extractor is a
// pure function (same text → same set of types), so the whole bench is reproducible.
//
// REUSE, DON'T REINVENT (ADR 0007). The 22-kind taxonomy + the marker vocabulary are LIFTED from
// the DG01 spike (spike/diffusiongemma, GO-conditional verdict) and re-graved here as real
// ratcheted code. We do NOT import the spike module (it is standalone, throwaway, has its own
// go.mod): the taxonomy is small, declared, and belongs above the line — it is copied, not linked.
...
// each split into the FACETS a spec can carry and a generation model can independently surface. The
// completeness measure is purely set arithmetic over THIS closed set — never a free-form judgement.
...
// AllKinds is the CLOSED taxonomy, in a fixed (deterministic) order. A spec's "expected" set is a
// subset of this; coverage is measured against that expected set, never against this whole list.
...
// present in an output line, signal that the output SURFACED that requirement type. These are the
// parser's closed vocabulary — NOT a model: the extractor is a pure string match (determinism-first;
// "a parse is a parse", never an LLM judging an LLM). An output is modeled as plain text (Markdown-
// ish requirement notes) because that is what a generation model emits and what a recompile would or
// would not also produce. Declared above the line, never learned.
...
// Extract maps an output text to the SET of requirement TYPES it surfaces. PURE and DETERMINISTIC:
// lower-cases, scans against the closed marker vocabulary, returns a sorted de-duplicated slice. No
// LLM, no clock, no rng, never panics. This is the function the bench re-runs over EVERY candidate
// output (the recompile baseline, model A, model B, …) so a model's output is always re-judged by
// deterministic code (§8 — the judge is deterministic; the LLM is the gated exception in DG04).
...
// asSet turns a slice of kinds into a set for union/diff math.
...
// union returns the de-duplicated, sorted union of several kind-sets. Deterministic order.
...
// diff returns sorted (a \ b): elements of a not in b. Deterministic order.
